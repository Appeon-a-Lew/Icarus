...
template <typename T> class Deque {
  public:
    ...
    template <typename... Args> void push_bottom(Args&&... args);
    ...
    std::optional<T> pop_bottom() noexcept;
    ...
    std::optional<T> pop_top() noexcept;
...
template <typename T> template <typename... Args> void Deque<T>::push_bottom(Args&&... args) {
...
template <typename T> std::optional<T> Deque<T>::pop_bottom() noexcept {
...
template <typename T> std::optional<T> Deque<T>::pop_top() noexcept {
    std::int64_t t = _top.load(acquire);
    std::atomic_thread_fence(seq_cst);
...
template <typename T> Deque<T>::~Deque() noexcept { delete _buffer.load(); }
